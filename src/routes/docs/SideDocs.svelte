...
<Drawer.Root bind:open={toggle} direction="right"
	...
	<Drawer.Portal>
		...
		<Drawer.Content 
			class="fixed md:w-[50%] w-[90%] right-0 top-0 flex bg-base-300 rounded-2xl border-secondary/50 border-l-2">
			<DarkLightToggle />

			<!-- Tab on the side -->
			<div class="p-2 -left-8 absolute bottom-[45%]">
			<div class="rounded-l-2xl h-24 w-6 bg-base-300 flex justify-center items-center border-secondary/50 border-2 border-r-0 gap-1">
