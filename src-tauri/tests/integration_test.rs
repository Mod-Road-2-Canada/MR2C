...
fn test_merge_images() {
    ...
    fs_extra::copy_items(
        &[base_path.join("tests/,moddedcompare/gfx")],
    ...
    merge_all_img_to_gfx(
        mod_gfx,
        &game_gfx.to_string_lossy().into_owned(),
    ...
    assert_two(
        &base_path.join("tests/mods/testmod/"),
        &base_path.join("tests/,moddedcompare/expected/"),
        &vec![".png".to_string(), ".dfmod".to_string()],
    );
}
