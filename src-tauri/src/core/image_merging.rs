...
use super::image_constant::{match_special_file, sheet_to_idx_size, IDX_TO_SHEET, SPRITE_TOTAL};
...
impl Indices {
    ...
    fn map_size_to_index(&mut self, search_png: &str, total_area: u32) -> Result<(), Error> {
        if let Some((i, dim)) = sheet_to_idx_size(search_png) {
            ...
            return Err(Error::ModImgError(ModImgError::SpriteMapNotFound(
                search_png.to_string(),
            )));
        }
    ...
    fn has_reached_max(&self) -> Result<(), Error> {
        ...
        if self.sprite_count[last_index] > 15200 {
            return Err(Error::ModImgError(ModImgError::TooManySprites(
                self.sprite_count[last_index],
            )));
        }
                ...
                if ex1 == "png" {
                    if let Some(filename) = path.file_stem() {
                        ...
                        let img = image::open(&path).with_context(|| {
                            format!("Mapping: Failed to open img 1: {}", path.display())
                        })?;
...
fn merge_two_images<P: AsRef<Path>>(img1_path: P, img2_path: P) -> Result<(u32, u32, u32), Error> {
    let img1 = image::open(&img1_path)
        .with_context(|| format!("MFailed to open img 1: {}", img1_path.as_ref().display()))?;
    let img2 = image::open(&img2_path)
        .with_context(|| format!("MFailed to open img 2: {}", img2_path.as_ref().display()))?;
    ...
    println!(
        "Merged {} to: {}",
        img2_path.as_ref().display(),
        img1_path.as_ref().display()
    );
...
fn merge_all_imgs_in_folder<P: AsRef<Path>>(
    mod_gfx: P,
    game_gfx: &str,
    mod_index: &mut Indices,
    all_index: &mut Indices,
) -> Result<u16, Error> {
    ...
    for entry in WalkDir::new(&mod_gfx) {
        ...
        if !destination.try_exists()? {
            continue;
        } // Skip files that don't exist in game gfx

        if let (Some(ex1), Some(ex2)) = (path.extension(), destination.extension()) {
            if ex1 == "png" && ex2 == "png" {
                let (width, total_height, mod_height) =
                    merge_two_images(&destination.as_path(), &path)?;

                let png_name = destination
                    .file_stem()
                    .ok_or(Error::Other(
                        "2. Cannot get file name (without extension).".to_string(),
                    ))?
                    .to_string_lossy()
                    .into_owned();
...
pub fn merge_all_img_to_gfx<P: AsRef<Path>>(
    mod_gfx: P,
    game_gfx: &str,
    index_file: &str,
    mod_tag: &str,
) -> Result<u16, Error> {
    if !mod_gfx.as_ref().is_dir() {
        custombail!("Not a directory: {}", mod_gfx.as_ref().display());
    }
...
pub fn overlap_in_images(img1_path: &str, img2_path: &str) -> Result<(), Error> {
    let img1 =
        image::open(&img1_path).with_context(|| format!("Failed to open img 1: {}", img1_path))?;
    let img2 =
        image::open(&img2_path).with_context(|| format!("Failed to open img 2: {}", img2_path))?;

    if img2.height() > img1.height() {
        custombail!(
            "Second image {} is larger the original image {}",
            img2_path,
            img1_path
        );
    }
    ...
    if img2.width() != img1.width() {
        custombail!(
            "Second image {} is not same width with the original image {}",
            img2_path,
            img1_path
        );
    }
