...
use app::core::func::{save, SavePosition, REMOVE};
...
pub fn load_mod(mod_file: &str, mod_tag: &str, mod_install_state: u8) -> Result<(), Error> {
    ...
    script = script.replace(
        "Save_This.",
        throw_on_err!("Savefile(File, Add, Search, Above, Bottom, mod_tag, mod_install_state);"),
    );
    script = script.replace(
        "Merge_This.",
        throw_on_err!("Mergefile(GfxFolder, IndexFile, mod_tag, mod_install_state);"),
    );
    script = script.replace(
        "Overlap_This.",
        throw_on_err!("Overlapfile(File, Add, mod_install_state);"),
    );
