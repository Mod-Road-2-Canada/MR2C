...
use crate::loader::image_constant::{SPRITE_TOTAL, summon_sprite_size_map};
...
impl Indices {
	...
	fn map_file_to_indices(&mut self, search_name: &str, total_area: u32) -> Result<(), Error> {
		if let Some((index, _, dim)) = summon_sprite_size_map().get_full(search_name) {
			if dim[0] > 0 {
			...
			return Err(Error::ModImgError(
				ModImgError::SpriteMapNotFound(search_name.to_string())
			));
		}
	...
	fn print_filtered_to_df_file(&self, filtered: &Vec<usize> , output_file: &str) -> Result<(), Error> {
		...
		for i in filtered.iter() {
			if let Some((sprite_name, _)) = summon_sprite_size_map().get_index(*i) {
				let line = format!("{} constant {}\n", self.sprite_count[*i], sprite_name);
				writer.write_all(line.as_bytes())?;
			} // Don't need an error here since this is covered
		}
